...
def get_proxy_filename():  
    """
    Return a proxy certificate filename.
    """
    ...
    if os.name == 'posix':
        ...
        try:
            filename = 'x509up_u%s' % os.environ['USERNAME']
        except KeyError:
            raise RuntimeError("USERNAME or X509_USER_PROXY must be set for NT platform. Can't determine proxy file location.")
    else:
